...
def threadFunc():
    ...
    while not endThreatFlag: #can be chane outside the function in order to break the loop and end Func:threadFunc
        speed = int(car.speed)
        if takeImagesWhileDriving and speed >= speedToSave:
            ...
            imageCounter += 1
            filename = 'IMG_{}_{}_{}_{:04d}_S{:03d}_A{:03d}.jpg'.format(runName,runID,currentTime,imageCounter,speed,angle)
...
def runSRC():
    try:
        ...
        message = 'Status:{} Info:{} ThreadCam:{}-{}'.format(status,infoStr,takeImagesWhileDrivingStr,imageCounter)
        print(message)
        while True:

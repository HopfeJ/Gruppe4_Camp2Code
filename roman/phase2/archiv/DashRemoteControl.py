...
def save_image(image_id, run_id, frame):
    ...
    path = './images/'
    filename = 'IMG_{}_{}_{}_{:04d}_S{:03d}_A{:03d}.jpg'.format('DRC',run_id,current_time,image_id,car.speed,car.steering_angle)
...
app.layout = html.Div(children=[
    ...
    html.Div([
        html.Button('Rücklenken der Frontmotoren deaktivieren', id='angle-button-press', n_clicks=0, style={
                    'font-size': '12px', 'width': '140px', 'display': 'inline-block', 'margin-bottom': '10px', 'margin-right': '5px', 'height': '37px', 'verticalAlign': 'top'}),
        html.Button('Klicken für die Bildaufnahme', id='take-images-button', n_clicks=0, style={
                    'font-size': '12px', 'width': '140px', 'display': 'inline-block', 'margin-bottom': '10px', 'margin-right': '5px', 'height': '37px', 'verticalAlign': 'top'}),
        html.Button('SERVER SHUTDOWN', id='stop-button-press', n_clicks=0, style={
                    'font-size': '12px', 'width': '140px', 'display': 'inline-block', 'margin-bottom': '10px', 'margin-right': '5px', 'height': '37px', 'verticalAlign': 'top'}),
    ]),
    ...
    html.Div([
        html.H2("Kamera Feed"),
        html.Img(src="/video_feed",style={'width': '30%', "border": "2px black solid"})
    ])
]
)
...
if __name__ == '__main__':
    ...
    s.close()
    app.run_server(host=ipaddress, port=8050, debug=True)

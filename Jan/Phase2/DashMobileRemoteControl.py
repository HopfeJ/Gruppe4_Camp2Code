...
# TAB1 Remote Control
list_of_switches = [dbc.Col(daq.BooleanSwitch(id='boolean-switch-joystick', on=False, label="J-Mode"))]
if hasCam:
          list_of_switches += [dbc.Col(daq.BooleanSwitch(id='boolean-switch-video', on=False, label="Video")),dbc.Col(daq.BooleanSwitch(id='boolean-switch-record', on=False, label="Record"))]
        ...
        dbc.Row([
                dbc.Col(
                    html.Div([
                        ...
                        ],vertical=True, size='sm', style={"width":"100%"}),
                        dbc.Row(list_of_switches)
                    ]),
                width=2, style={"border":"1px black solid","width":"23vw"}),
                dbc.Col(
                    html.Div([
                        html.H6("Sp."),
                        daq.Slider(id='slider-maxspeed' ,min=0, max=100, value=30,size=180,
                                   vertical=True,
                                   handleLabel={"showCurrentValue": True,"label": "Speed"}),     
                    ]),
                width=1, style={"border":"1px black solid","width":"7vw"}),
                dbc.Col(
                    html.Div([
                        html.H6("Info"),
                        html.Div(listOfConfigs,id='live-view'),
                        html.Output(id="status-message2", style={"verticalAlign": "middle","color": "red"})
                    ]),
                width=5, style={"border":"1px black solid","width":"45vw"}),
                dbc.Col(
                    html.Div([
                        html.H6('Front wheels'),
                        daq.Joystick(id='joystick',label="Steering Angel",angle=0,size=160),
                        html.Output('output',id="output-joystick", style={"verticalAlign": "middle","color": "red"})
                    ]),
                width=3, style={"border":"1px black solid","width":"25vw"}),
            ])
    ],fluid=True, style={"height": "90vh","width": "100vw"})
...
app.layout = html.Div([
    dbc.Row([
        dbc.Col(
            html.H3("PiCar Control")
            ,width = 5),
        dbc.Col(
            html.Output(id="status-message")
            ,width = 5),
        dbc.Col(
            dbc.Button('SHUTDOWN',id='B-shutdown', n_clicks=0, size = 'sm',style = {'width': '100%'})
            ,width = 2)
    ]),
    tabs_content,
    ...
    dcc.Store(id='shutdown',storage_type='session')],style={"height": "100vh","width": "100vw"})
...
def switch_video(videoSwitch,recordSwitch):
    ...
    if videoSwitch and hasCam:
        videoElement = [html.Img(src="/video_feed",style={"border":"1px black solid","width":"100%"})]
    else:
...
def clicks(n1, n2, n3, j_angle, j_force, s_maxspeed, angle, speed, driving, controltype):
    #triggered_id = ctx.triggered_id # used in older dash version
    triggered_id = callback_context.triggered_id
    print('EVENT:','trigger',triggered_id,'-',n1, n2, n3, j_angle, j_force, s_maxspeed, angle,speed,driving,controltype)
    speed = speed or 0 
    ...
    button_stop_style, button_forward_style, button_backward_style = [black_button_style]*3
    steeringtype = 'direct'
    #
    if triggered_id == 'joystick':
        if controltype: # speed and angle by joystick
            ...
            message_joystick='({},{},{})'.format(steeringtype,round(j_force,2),round(j_angle,2))
...
# shutdown button
@app.callback(
    Output('main','children'),Output('B-shutdown','style'),Input('B-shutdown','n_clicks')
)
def shutdown_server(n_clicks):
    ...
    if n_clicks > 0:
        ...
        func()
        return html.Div('Server is down! Restart server at the Raspberry to continue.',style={'textAlign': 'center'}),{'background-color': 'red'}
    else:
        # can't be reached
...
def generate_camera_image(cam):
    ...
    while True:
        ...
        frame = cam.get_frame()
        cam.imageNumber += 1
        if cam.recording and car.speed > cam.recordSpeed and cam.imageNumber % cam.k == 0:
            currentTime = datetime.now().strftime("%Y%m%d_%H-%M-%S")
            
            status = car.get_status()
            filename = 'IMG_{}_{}_{}_{:04d}_S{:03d}_A{:03d}.jpg'.format(cam.runName,cam.runID,currentTime,cam.imageNumber,status['speed'],status['angle'])
...
def video_feed():
    ...
    """
    print('video-feed')
    return Response(generate_camera_image(cam),
